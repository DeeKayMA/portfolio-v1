...
export function CertificateCard({ certificate, company, description, link, year }: CertificateCardProps) {
    ...
    role="link"
        ...
        <div className="flex flex-col gap-4 w-full lg:w-3/4">
            ...
            {/* education Name */}
            <h3 className={`font-medium flex-wrap text-[#FFFFFF] group-hover/education:text-amber-400 flex flex-row items-center gap-1`}>{certificate}<span className="text-2xl px-1">·</span>{company}{(link) && arrowUpRightSVG}</h3>
